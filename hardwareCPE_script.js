const {dbConnectHardwareCPE, dbConnectCVE, createNewHardwareCPECollection} = require("../mongo_connect.js");
...
async function runScript()
{
  try
  {
    ...
    const cpeArray = await getCPEArray(cpeNames, collection1, collection2, size);
  }
  catch (error)
  {
    console.log(error);
...
async function getAllCPENames(collection1)
{
  try
  {
    const cpeNames = await collection1.distinct("model");
    return cpeNames;
  }
  catch (error)
  {
...
{
  const newCollection = await createNewHardwareCPECollection();
